<script lang="ts">
	...
	import { trash, view, cross } from '$lib/assets/action-button-icons';
	import { removeActivity } from '$lib/utils/adminActions.js';
...
<h1 class="mb-4 font-custom text-2xl">Manage Activities</h1>
<div class="text-base">
					...
					{#each pagination.displayPage() as row}
						{#key row.id}
							<tr>
								...
								<td class="flex h-12 w-36 items-center justify-center p-1">
								...
								<td>
									<div class="flex justify-center space-x-2">
										...
										<ActionButton
											imgsrc={trash}
											action={() => removeActivity(row.id, pagination)}
											alt={`Remove activity ${row.id}`}
											width={'20px'}
										></ActionButton>
									</div>
								</td>
