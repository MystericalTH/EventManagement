<script lang="ts">
	...
	let pagination: Pagination<Member> = createPagination<Member>(
		data.memberRequestList,
		10
	);
</script>
			...
			<table class="min-w-[794px] divide-y divide-gray-200">
				<thead class="bg-gray-100">
					<tr>
						...
						>
						<th
							class="w-40 min-w-36 px-3 py-3 text-left text-xs font-medium uppercase tracking-wider text-gray-500"
							>Phone Number</th
						>
						<th
							...
							<tr>
								...
								<td>
									<div class="flex justify-center space-x-2">
										<ActionButton
											imgsrc={checkCircle}
											action={() => approveRequest(row.id, pagination)}
											alt={`Approve request ${row.id}`}
											width={'20px'}
										></ActionButton>
										<ActionButton
											imgsrc={trash}
											action={() => rejectRequest(row.id, pagination)}
