...
func getActivity(w http.ResponseWriter, r *http.Request) {
	...
	err := db.QueryRow("SELECT id, title, proposer, startDate, endDate, maxNumber, format, description, proposeDateTime, acceptAdmin, acceptDateTime, applicationStatus FROM activities WHERE id = ?", r.URL.Query().Get("activityID")).Scan(
...
func deleteActivity(w http.ResponseWriter, r *http.Request) {
	activityID, _ := strconv.Atoi(r.URL.Query().Get("activityID"))
